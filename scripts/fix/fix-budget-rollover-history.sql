...
-- 
-- 功能：
-- 1. 为启用了结转的预算创建缺失的结转历史记录
-- 2. 更新不正确的预算结转金额
-- 
-- 使用方法：
-- 1. 连接到数据库：psql -h 数据库地址 -U 用户名 -d 数据库名
-- 2. 执行脚本：\i fix-budget-rollover-history.sql
-- 或者：psql -h 数据库地址 -U 用户名 -d 数据库名 < fix-budget-rollover-history.sql
...
-- 创建临时函数来检查是否存在结转历史记录
CREATE OR REPLACE FUNCTION has_rollover_history(budget_id_param TEXT, period_param TEXT) RETURNS BOOLEAN AS $$
DECLARE
...
BEGIN
    ...
    LOOP
        ...
        -- 计算当前预算的支出
        spent_amount := calculate_spent_amount(budget_record.id);
        ...
        IF budget_record.end_date < CURRENT_DATE THEN
            period_str := budget_record.end_date::DATE::TEXT;
            period_str := EXTRACT(YEAR FROM budget_record.end_date) || '-' || EXTRACT(MONTH FROM budget_record.end_date);
            
            IF NOT has_rollover_history(budget_record.id, period_str) THEN
                -- 创建结转历史记录
...
DROP FUNCTION IF EXISTS has_rollover_history(TEXT, TEXT);
