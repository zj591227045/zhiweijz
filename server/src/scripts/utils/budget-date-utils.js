...
exports.BudgetDateUtils = void 0;
class BudgetDateUtils {
     ...
     * @returns 预算周期
     */
    static calculateBudgetPeriod(year, month, refreshDay) {
        // 验证refreshDay
     ...
     * @returns 当前预算周期
     */
    static getCurrentBudgetPeriod(currentDate, refreshDay) {
     ...
     * @returns 下一个预算周期
     */
    static getNextBudgetPeriod(currentPeriod) {
     ...
     * @returns 上一个预算周期
     */
    static getPreviousBudgetPeriod(currentPeriod) {
     ...
     * @returns 缺失的预算周期数组
     */
    static calculateMissingPeriods(lastEndDate, currentDate, refreshDay) {
        const periods = [];
        // 从最后一个预算结束后的下一个周期开始
        ...
        while (checkDate <= currentDate) {
            ...
            // 检查是否已经添加过这个周期
            const exists = periods.some((p) => p.year === period.year && p.month === period.month && p.refreshDay === period.refreshDay);
            if (!exists) {
                periods.push(period);
            }
            // 移动到下一个周期的开始
     ...
     * @returns 是否有效
     */
    static isValidRefreshDay(refreshDay) {
     ...
     * @returns 天数
     */
    static calculatePeriodDays(period) {
     ...
     * @returns 剩余天数
     */
    static getRemainingDays(period, currentDate = new Date()) {
...
exports.BudgetDateUtils = BudgetDateUtils;
