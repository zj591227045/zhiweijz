...
class BudgetRepository {
     ...
     * 根据用户ID和家庭ID查找预算
     */
    async findByUserAndFamily(userId, familyId) {
     ...
     * 根据托管成员ID和家庭ID查找预算
     */
    async findByFamilyMemberAndFamily(familyMemberId, familyId) {
        return prisma.budget.findMany({
            where: {
                familyMemberId,
                familyId,
     ...
     */
    async calculateMemberSpentAmount(budgetId, memberId, startDate, endDate, isCustodial = false) {
        // 获取预算信息
    ...
    async create(userId, budgetData) {
        return prisma.budget.create({
            data: {
                ...
                amount: new client_1.Prisma.Decimal(budgetData.amount),
                ...
                budgetType: budgetData.budgetType || client_1.BudgetType.PERSONAL,
    ...
    async findAll(userId, params) {
        const { period, categoryId, familyId, active, budgetType, page = 1, limit = 20, sortBy = 'startDate', sortOrder = 'desc', } = params;
    ...
    async update(id, budgetData) {
        const data = {
            ...
            ...(budgetData.amount !== undefined && { amount: new client_1.Prisma.Decimal(budgetData.amount) }),
     ...
     * 可选根据账本ID进行过滤
     */
    async findActiveBudgets(userId, date = new Date(), accountBookId) {
     ...
     * 根据账本ID查找活跃的预算
     */
    async findActiveByAccountBookId(accountBookId, date = new Date()) {
     ...
     */
    async findByPeriodAndDate(userId, period, startDate, endDate, familyId, accountBookId, excludeFamilyMember = true) {
