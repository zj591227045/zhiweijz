 ...
 * 
 * 使用方法：
 * node manual-budget-rollover-fixed.js [--dry-run] [--enable-rollover]
 */
...
async function manualBudgetRollover(enableRollover = false, dryRun = false) {
  ...
  try {
    ...
    for (const budget of july2025Budgets) {
      ...
      console.log(`   当前结转状态: ${budget.rollover ? '已启用' : '未启用'}`);
      ...
      try {
        ...
        if (dryRun) {
          ...
          // 模拟计算结转金额
          console.log(`[试运行] 模拟计算结转金额...`);
          console.log(`   预算金额: ${budget.amount}`);
          console.log(`   上期结转: ${budget.rolloverAmount || 0}`);
          console.log(`   需要查询实际支出金额来计算结转`);
        ...
        // 这里需要调用BudgetService的方法，但由于我们在简化脚本中，
        // 我们先创建8月份预算，结转金额暂时设为0
        console.log(`📅 创建8月份预算...`);
        
        const august2025StartDate = new Date('2025-08-01');
        const august2025EndDate = new Date('2025-08-31');
        ...
        const newBudget = await prisma.budget.create({
          data: {
            ...
            period: BudgetPeriod.MONTHLY,
            startDate: august2025StartDate,
            endDate: august2025EndDate,
            ...
            enableCategoryBudget: budget.enableCategoryBudget,
            isAutoCalculated: budget.isAutoCalculated,
            refreshDay: budget.refreshDay,
            rolloverAmount: 0 // 暂时设为0，后续可以手动调整
          }
        });

        stats.successfulRollovers++;
        console.log(`✅ 预算 ${budget.name} 结转完成，新预算ID: ${newBudget.id}`);
