...
exports.BudgetSchedulerService = void 0;
...
class BudgetSchedulerService {
     ...
     * 建议在每月1号凌晨执行
     */
    async createMonthlyBudgetsForAllUsers() {
        try {
     ...
     * 包括注册用户和托管成员
     */
    async findUsersNeedingCurrentPeriodBudgets(currentDate) {
        // 1. 查找注册用户的历史预算
        ...
        for (const userBudget of allUsersWithBudgets) {
            if (!userBudget.userId || !userBudget.accountBookId)
                continue;
            const refreshDay = userBudget.refreshDay || 1;
            // 计算当前预算周期
            const currentPeriod = budget_date_utils_1.BudgetDateUtils.getCurrentBudgetPeriod(currentDate, refreshDay);
            // 检查是否已有当前周期的预算
     ...
     */
    async processExpiredBudgetRollovers() {
        try {
...
exports.BudgetSchedulerService = BudgetSchedulerService;
