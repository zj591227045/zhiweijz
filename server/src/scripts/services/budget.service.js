...
const category_budget_repository_1 = require("../repositories/category-budget.repository");
const budget_date_utils_1 = require("../utils/budget-date-utils");
// 创建Prisma客户端实例
const prisma = new client_1.PrismaClient();
const budget_model_1 = require("../models/budget.model");
const category_model_1 = require("../models/category.model");
const category_budget_model_1 = require("../models/category-budget.model");
class BudgetService {
    constructor() {
        ...
        this.categoryBudgetRepository = new category_budget_repository_1.CategoryBudgetRepository();
    }
    /**
     * 创建预算
     */
    async createBudget(userId, budgetData) {
        ...
        if (budgetData.budgetType === client_1.BudgetType.PERSONAL && budgetData.accountBookId) {
            const existingBudget = await this.checkExistingBudget(userId, budgetData.accountBookId, budgetData.budgetType, budgetData.period, budgetData.startDate, budgetData.familyMemberId);
            if (existingBudget) {
        ...
        return (0, budget_model_1.toBudgetResponseDto)(budget, budget.category ? (0, category_model_1.toCategoryResponseDto)(budget.category) : undefined);
    }
    /**
     * 获取预算列表
     */
    async getBudgets(userId, params) {
        ...
        if (params.budgetType === 'PERSONAL' && params.accountBookId) {
            ...
            const currentMonthBudgets = await this.budgetRepository.findByPeriodAndDate(userId, client_1.BudgetPeriod.MONTHLY, new Date(currentDate.getFullYear(), currentDate.getMonth(), 1), new Date(currentDate.getFullYear(), currentDate.getMonth() + 1, 0), undefined, params.accountBookId, true);
        ...
        const budgetsWithSpent = await Promise.all(budgets.map(async (budget) => {
            ...
            // 创建预算响应DTO
            const budgetDto = (0, budget_model_1.toBudgetResponseDto)(budget, budget.category ? (0, category_model_1.toCategoryResponseDto)(budget.category) : undefined, spent);
            // 添加用户名称信息
            if (budget.user) {
            ...
            try {
                ...
                if (budgetDto.familyMemberId) {
                    ...
                    if (budget.familyMember) {
                        ...
                        budgetDto.familyMemberName = budget.familyMember.name;
                    }
                    else {
                        // 否则查询托管成员信息
    ...
    async getBudgetById(id, userId) {
        ...
        if (budget.enableCategoryBudget && budget.categoryBudgets) {
            categoryBudgets = budget.categoryBudgets.map((cb) => (0, category_budget_model_1.toCategoryBudgetResponseDto)(cb, cb.category ? (0, category_model_1.toCategoryResponseDto)(cb.category) : undefined));
        }
        const result = (0, budget_model_1.toBudgetResponseDto)(budget, budget.category ? (0, category_model_1.toCategoryResponseDto)(budget.category) : undefined, spent);
        // 添加分类预算到响应
    ...
    async updateBudget(id, userId, budgetData) {
        ...
        if (budgetData.amount !== undefined && Number(budget.amount) !== budgetData.amount) {
            // 只对个人月度预算应用修改限制
            if (budget.period === client_1.BudgetPeriod.MONTHLY &&
                budget.budgetType === client_1.BudgetType.PERSONAL) {
                // 检查是否已经修改过预算金额
                if (budget.amountModified) {
        ...
        if (updatedBudget.enableCategoryBudget && updatedBudget.categoryBudgets) {
            categoryBudgets = updatedBudget.categoryBudgets.map((cb) => (0, category_budget_model_1.toCategoryBudgetResponseDto)(cb, cb.category ? (0, category_model_1.toCategoryResponseDto)(cb.category) : undefined));
        }
        const result = (0, budget_model_1.toBudgetResponseDto)(updatedBudget, updatedBudget.category ? (0, category_model_1.toCategoryResponseDto)(updatedBudget.category) : undefined, spent);
        // 添加分类预算到响应
        result.categoryBudgets = categoryBudgets;
        return result;
    }
     ...
     * @returns 该日期范围内的预算列表
     */
    async getBudgetsByDate(userId, date, accountBookId) {
        try {
            ...
            for (const budget of budgets) {
                const spent = await this.budgetRepository.calculateSpentAmount(budget.id);
                const category = budget.category ? (0, category_model_1.toCategoryResponseDto)(budget.category) : undefined;
                const budgetDto = (0, budget_model_1.toBudgetResponseDto)(budget, category, spent);
                // 添加额外的计算字段
     ...
     * 如果没有找到当前月份的预算，自动创建
     */
    async getActiveBudgets(userId, accountBookId) {
        // 先尝试查询当前月份的预算
     ...
     */
    async getBudgetRolloverHistoryByBudgetId(budgetId, userId) {
        // 检查预算是否存在
        ...
        SELECT
          bh.id, bh.budget_id as "budgetId",
      ...
      `;
            ...
            if (histories.length > 0) {
                ...
                histories.slice(0, 3).forEach((h, index) => {
            ...
            rolloverHistory.slice(0, 3).forEach((h, index) => {
     ...
     */
    async processBudgetRollover(budgetId, options = {}) {
        // 获取预算信息
        const budget = await this.budgetRepository.findById(budgetId);
        if (!budget) {
            throw new Error('预算不存在');
     ...
     * 处理复杂的记账场景和边界情况
     */
    async calculateEnhancedSpentAmount(budgetId, options = {}) {
        const budget = await this.budgetRepository.findById(budgetId);
        if (!budget) {
            return 0;
        }
        ...
        if (options.validateTransactionDates) {
            whereConditions.date.gte = new Date(Math.max(whereConditions.date.gte.getTime(), budget.startDate.getTime()));
            whereConditions.date.lte = new Date(Math.min(whereConditions.date.lte.getTime(), budget.endDate.getTime()));
        }
        // 查询记账记录
        ...
        console.log(`支出计算完成: 有效记账 ${validTransactionCount} 笔, 异常记账 ${invalidTransactionCount} 笔, 总支出: ${totalSpent}`);
        return totalSpent;
    }
    /**
     * 验证结转计算结果
     */
    async validateRolloverCalculation(budget, spent, rolloverAmount) {
        try {
            const amount = Number(budget.amount);
            const currentRolloverAmount = Number(budget.rolloverAmount || 0);
            const totalAvailable = amount + currentRolloverAmount;
     ...
     * 详细记录结转过程，便于审计和问题排查
     */
    async recordBudgetRolloverHistory(budget, spent, currentRolloverAmount, rolloverAmount) {
        try {
     ...
     */
    async calculateSmartRolloverAmount(budgetId, options = {}) {
        const budget = await this.budgetRepository.findById(budgetId);
     ...
     * 处理债务结转逻辑
     */
    async handleDebtRollover(budget, debtAmount, options) {
     ...
     * 处理余额结转逻辑
     */
    async handleSurplusRollover(budget, surplusAmount, options) {
     ...
     * 判断是否应该清零债务
     */
    async shouldClearDebt(budget, debtAmount) {
        // 小额债务自动清零（可配置阈值）
     ...
     * 记录债务清零
     */
    async recordDebtClearance(budget, debtAmount) {
        try {
     ...
     * 记录债务结转
     */
    async recordDebtRollover(budget, debtAmount) {
        try {
            await prisma.budgetHistory.create({
                data: {
                    budgetId: budget.id,
     ...
     * 记录余额结转
     */
    async recordSurplusRollover(budget, surplusAmount) {
        try {
            await prisma.budgetHistory.create({
                data: {
                    budgetId: budget.id,
     ...
     */
    async recalculateBudgetRollover(budgetId, recalculateChain = true) {
        // 获取预算信息
        const budget = await this.budgetRepository.findById(budgetId);
        if (!budget) {
            throw new Error('预算不存在');
     ...
     */
    async recalculateBudgetRolloverChain(startBudgetId) {
     ...
     * 重新计算单个预算的结转（不影响后续预算）
     */
    async recalculateSingleBudgetRollover(budgetId) {
        const budget = await this.budgetRepository.findById(budgetId);
        if (!budget) {
            throw new Error('预算不存在');
        }
     ...
     * 更新或创建预算历史记录
     */
    async updateBudgetHistory(budget, spent, previousRollover) {
        const amount = Number(budget.amount);
        ...
        if (existingHistory) {
            ...
            if (endDate < currentDate) {
                ...
                console.log(`创建历史记录: ${period}, 结转金额: ${remaining}, 预算ID: ${budget.id}`);
            }
        }
    }
    /**
     * 检查是否已存在相同条件的预算
     */
    async checkExistingBudget(userId, accountBookId, budgetType, period, startDate, familyMemberId) {
        try {
     ...
     * 用于防止在家庭账本中创建重复的个人预算
     */
    async checkExistingPersonalBudgetInAccountBook(userId, accountBookId, period, startDate) {
        try {
            const existingBudget = await prisma.budget.findFirst({
                where: {
                    userId,
     ...
     * @returns 是否有权限
     */
    async checkBudgetAccess(userId, budget) {
        // 如果是用户自己的预算，直接允许
     ...
     * @returns 是否有权限
     */
    async checkAccountBookAccess(userId, accountBookId) {
        const accountBook = await prisma.accountBook.findUnique({
     ...
     * @returns 趋势数据数组
     */
    async getBudgetTrends(budgetId, viewMode = 'monthly', familyMemberId) {
     ...
     * @returns 趋势数据数组
     */
    async getAggregatedBudgetTrends(aggregatedBudgetId, viewMode = 'monthly', familyMemberId) {
        // 从聚合预算ID中提取用户ID
        ...
        for (const month of months) {
            try {
                ...
                for (const budget of userBudgets) {
                    ...
                    // 累加记账金额
                    const budgetAmount = transactions.reduce((sum, transaction) => sum + Number(transaction.amount), 0);
     ...
     * 包括用户个人预算和托管成员预算
     */
    async autoCreateMissingBudgets(userId, accountBookId) {
        try {
            console.log(`开始为用户 ${userId} 在账本 ${accountBookId} 中自动创建缺失预算`);
            // 首先检查当前月份是否已有个人预算，避免重复创建
            const currentDate = new Date();
            const startDate = new Date(currentDate.getFullYear(), currentDate.getMonth(), 1);
            const existingCurrentBudget = await this.checkExistingPersonalBudgetInAccountBook(userId, accountBookId, client_1.BudgetPeriod.MONTHLY, startDate);
            // 1. 处理用户个人预算
            console.log(`开始为用户 ${userId} 创建缺失的个人预算`);

            if (existingCurrentBudget) {
                console.log(`用户 ${userId} 在账本 ${accountBookId} 中已存在当前月份的个人预算: ${existingCurrentBudget.id}，跳过个人预算创建`);
            } else {
                if (latestPersonalBudget) {
                    const refreshDay = latestPersonalBudget.refreshDay || 1;
                    ...
                    if (latestEndDate < currentDate) {
                        const periodsToCreate = budget_date_utils_1.BudgetDateUtils.calculateMissingPeriods(latestEndDate, currentDate, refreshDay);
                        ...
                        for (const period of periodsToCreate) {
                            console.log(`创建个人预算周期: ${budget_date_utils_1.BudgetDateUtils.formatPeriod(period)}`);
                            // 处理上个预算的结转（如果启用了结转）
            ...
            if (accountBook?.familyId) {
                ...
                for (const member of custodialMembers) {
                    ...
                    if (latestCustodialBudget) {
                        const refreshDay = latestCustodialBudget.refreshDay || 1;
                        ...
                        if (latestEndDate < currentDate) {
                            ...
                            for (const period of periodsToCreate) {
                                console.log(`为托管成员 ${member.name} 创建预算周期: ${budget_date_utils_1.BudgetDateUtils.formatPeriod(period)}`);
                                // 处理上个预算的结转（如果启用了结转）
                                let rolloverAmount = 0;
     ...
     * 查找用户最近的个人预算
     */
    async findLatestPersonalBudget(userId, accountBookId) {
        const budgets = await prisma.budget.findMany({
            where: {
                userId,
                accountBookId,
     ...
     * 用于记账时检查，如果没有预算则自动创建
     */
    async ensureCurrentMonthBudget(userId, accountBookId) {
        const currentDate = new Date();
        const startDate = new Date(currentDate.getFullYear(), currentDate.getMonth(), 1);
        // 使用新的检查方法，不考虑familyMemberId
        const existingBudget = await this.checkExistingPersonalBudgetInAccountBook(userId, accountBookId, client_1.BudgetPeriod.MONTHLY, startDate);
        if (!existingBudget) {
            console.log(`用户 ${userId} 在账本 ${accountBookId} 中没有当前月份的个人预算，记账时自动创建`);
            await this.autoCreateMissingBudgets(userId, accountBookId);
        }
    }
    /**
     * 为用户创建当月默认个人预算
     */
    async createDefaultPersonalBudget(userId, accountBookId) {
        try {
            // 获取账本信息以确定是否为家庭账本
            const accountBook = await prisma.accountBook.findUnique({
                where: { id: accountBookId },
     ...
     * 查找托管成员最近的预算
     */
    async findLatestCustodialMemberBudget(memberId, accountBookId) {
        const budgets = await prisma.budget.findMany({
            where: {
     ...
     * @returns 新创建的预算
     */
    async createBudgetForPeriod(templateBudget, period, rolloverAmount = 0) {
        // 检查是否已存在相同条件的预算
        const userId = templateBudget.userId || '';
        const accountBookId = templateBudget.accountBookId || '';
        const budgetType = templateBudget.budgetType || client_1.BudgetType.PERSONAL;
        const familyMemberId = templateBudget.familyMemberId;
        if (userId && accountBookId) {
            const existingBudget = await this.checkExistingBudget(userId, accountBookId, budgetType, templateBudget.period, period.startDate, familyMemberId);
            if (existingBudget) {
                console.log(`预算周期 ${period.startDate.toISOString()} 已存在预算: ${existingBudget.id}，跳过创建`);
                return (0, budget_model_1.toBudgetResponseDto)(existingBudget);
            }
        }
        const newBudgetData = {
            ...
            budgetType: templateBudget.budgetType || client_1.BudgetType.PERSONAL,
            familyMemberId: templateBudget.familyMemberId || undefined,
        ...
        if (templateBudget.rollover) {
            ...
            if (updatedBudget) {
                return (0, budget_model_1.toBudgetResponseDto)(updatedBudget);
            }
        }
        return newBudget;
    }
     ...
     * 为预算创建结转历史记录（如果需要）
     */
    async createRolloverHistoryIfNeeded(templateBudget, rolloverAmount) {
        // 只为已过期的预算创建历史记录
        const currentDate = new Date();
     ...
     * 对于月度预算，创建下个月的预算
     */
    async createNextPeriodBudget(budgetId) {
        // 获取当前预算
        ...
        // 只处理个人预算
        if (currentBudget.budgetType !== client_1.BudgetType.PERSONAL) {
            throw new Error('只能为个人预算创建下一周期');
        }
        const refreshDay = currentBudget.refreshDay || 1;
        if (currentBudget.period === client_1.BudgetPeriod.MONTHLY) {
            // 根据当前预算的结束日期计算下一个周期
            const currentEndDate = new Date(currentBudget.endDate);
            const nextPeriod = budget_date_utils_1.BudgetDateUtils.calculateMissingPeriods(currentEndDate, new Date(currentEndDate.getTime() + 24 * 60 * 60 * 1000), // 下一天
            refreshDay)[0];
            if (!nextPeriod) {
            ...
            const newBudgetData = {
                ...
                accountBookId: currentBudget.accountBookId || undefined,
                enableCategoryBudget: currentBudget.enableCategoryBudget || false,
                isAutoCalculated: currentBudget.isAutoCalculated || false,
                budgetType: client_1.BudgetType.PERSONAL,
