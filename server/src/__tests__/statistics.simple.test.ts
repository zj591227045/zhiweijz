...
describe('StatisticsService', () => {
  ...
  beforeEach(() => {
    ...
    // 模拟私有方法
    (statisticsService as any).groupTransactionsByDate = jest
      .fn()
      .mockImplementation((transactions, groupBy) => {
        if (groupBy === 'day') {
          return [
          ...
          return [{ date: '2023-01', amount: 800 }];
        }
        return [];
      });

    (statisticsService as any).groupTransactionsByCategory = jest
      .fn()
      .mockImplementation((transactions, categories, total) => {
        return [
          {
            category: {
              id: 'category-id-1',
    ...
    (statisticsService as any).calculateBudgetByCategory = jest
      .fn()
      .mockImplementation((budgets, transactions, categories) => {
        return [
          {
            category: {
              id: 'category-id-1',
    ...
    (statisticsService as any).getCategoriesMap = jest
      .fn()
      .mockImplementation((userId, familyId) => {
    ...
    (statisticsService as any).isUserFamilyMember = jest
      .fn()
      .mockImplementation((userId, familyId) => {
    ...
    (statisticsService as any).budgetRepository = {
      findByPeriodAndDate: jest
        .fn()
        .mockImplementation((userId, period, startDate, endDate, familyId) => {
          return [
            {
              id: 'budget-id-1',
              name: 'Budget 1',
  ...
  describe('groupTransactionsByDate', () => {
    it('should group transactions by day', () => {
      ...
      // 确保原始方法可用
      (statisticsService as any).groupTransactionsByDate = function (
        transactions: any[],
        groupBy: string,
      ) {
