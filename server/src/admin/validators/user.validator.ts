...
export const CreateUserSchema = z.object({
  ...
  email: z.string().email('邮箱格式不正确').max(100, '邮箱长度不能超过100个字符'),

  password: z.string().min(6, '密码长度至少6个字符').max(50, '密码长度不能超过50个字符'),

  bio: z.string().max(200, '个人简介长度不能超过200个字符').optional(),
});
...
export const UpdateUserSchema = z.object({
  ...
  email: z.string().email('邮箱格式不正确').max(100, '邮箱长度不能超过100个字符').optional(),

  bio: z.string().max(200, '个人简介长度不能超过200个字符').optional(),

  dailyLlmTokenLimit: z
    .number()
    .int('Token限额必须是整数')
    .min(0, 'Token限额不能为负数')
...
export const ResetPasswordSchema = z.object({
  newPassword: z.string().min(6, '密码长度至少6个字符').max(50, '密码长度不能超过50个字符'),
});
