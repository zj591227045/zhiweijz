...
export class MinIOInitializationService {
  ...
  private async waitForMinIOReady(): Promise<void> {
    ...
    for (let i = 0; i < maxRetries; i++) {
      try {
        ...
        if (response.ok) {
          logger.info(`✅ MinIO服务已就绪 (尝试 ${i+1}/${maxRetries})`);
          return;
        }
        logger.info(`⏳ MinIO服务尚未就绪，等待中... (尝试 ${i+1}/${maxRetries})`);
      } catch (error) {
        logger.info(`⏳ 无法连接到MinIO，等待中... (尝试 ${i+1}/${maxRetries})`);
      }
    ...
    logger.warn('⚠️ 等待MinIO就绪超时，将继续尝试初始化');
  }
  ...
  private async checkMinIOAvailability(): Promise<boolean> {
    try {
      ...
      if (isHealthy) {
        logger.info('✅ MinIO服务健康检查通过');
        return true;
      } else {
        logger.info('❌ MinIO服务健康检查失败');
        return false;
      }
    } catch (error) {
      logger.error('MinIO服务可用性检查失败:', error);
      return false;
    }
  }
  ...
  private async generateAccessKeys(): Promise<{ accessKeyId: string; secretAccessKey: string }> {
    ...
    try {
      ...
      logger.info(`✅ 使用MinIO root用户凭据作为访问密钥`);
      logger.info(`✅ 访问密钥ID: ${accessKeyId}`);
      logger.info(`✅ 访问密钥: ${secretAccessKey.substring(0, 4)}...`);

      // 验证凭据是否有效
      try {
        await this.validateCredentials(accessKeyId, secretAccessKey);
      } catch (error) {
        logger.error('❌ 凭据验证失败:', error);
        throw new Error('MinIO凭据无效，请检查环境变量配置');
      }

      return { accessKeyId, secretAccessKey };
    } catch (error) {
      logger.error('配置访问密钥失败:', error);
  ...
  private async validateCredentials(accessKeyId: string, secretAccessKey: string): Promise<void> {
    logger.info(`验证凭据有效性: ${accessKeyId}`);
    ...
    try {
      ...
      if (!isValid) {
        try {
          logger.info('尝试列出所有存储桶来验证凭据...');
          const command = new ListBucketsCommand({});
          const result = await testClient.send(command);
          if (result.Buckets) {
            logger.info(`✅ 凭据验证成功（通过ListBuckets）, 找到 ${result.Buckets.length} 个存储桶`);
            isValid = true;
          }
        } catch (error: any) {
          if (error.name === 'AccessDenied' || error.$metadata?.httpStatusCode === 403) {
            ...
            logger.info('❌ 通过ListBuckets验证失败:', error.name);
          }
        }
      }
      ...
      if (!isValid && accessKeyId === this.MINIO_ROOT_USER) {
        logger.info('⚠️ 无法验证root凭据，但将假设它是有效的');
        isValid = true;
      }
        ...
        try {
          ...
          if (error.name === 'NotFound' || error.$metadata?.httpStatusCode === 404) {
            ...
            try {
              ...
              logger.error(`❌ 创建存储桶 ${bucket} 失败:`, createError);

              // 如果使用新凭据失败，尝试使用root凭据
              try {
                logger.info(`尝试使用root凭据创建存储桶 ${bucket}...`);
                const createCommand = new CreateBucketCommand({ Bucket: bucket });
                await this.s3Client.send(createCommand);
                logger.info(`✅ 存储桶 ${bucket} 使用root凭据创建成功`);
                createdBuckets.push(bucket);
              } catch (rootCreateError) {
                logger.error(`❌ 使用root凭据创建存储桶 ${bucket} 也失败:`, rootCreateError);
              }
            }
          } else {
            logger.error(`❌ 检查存储桶 ${bucket} 失败:`, error);
          }
        }
      }

      // 跳过设置存储桶权限（在容器环境中无法使用mc命令）
      logger.info('⚠️ 跳过设置存储桶权限（容器环境限制）');

      return createdBuckets;
    } catch (error) {
      logger.error('创建存储桶过程失败:', error);
  ...
  async testMinIOConnection(): Promise<boolean> {
    try {
      ...
      logger.error('MinIO连接测试失败:', error);
      return false;
    }
  }
}
