  ...
  const loadData = async () => {
    try {
      ...
      const [configRes, statsRes, statusRes, templatesRes] = await Promise.all([
        ...
        adminApiClient.get(
          `${ADMIN_API_ENDPOINTS.STORAGE_CONFIG.replace('/config', '/status')}?t=${timestamp}`,
        ), // 使用管理员API
        adminApiClient.get(
          `${ADMIN_API_ENDPOINTS.STORAGE_CONFIG.replace('/config', '/templates')}?t=${timestamp}`,
        ),
      ]);
  ...
  const diagnoseStorage = async () => {
    try {
      const response = await adminApiClient.get(
        `${ADMIN_API_ENDPOINTS.STORAGE_CONFIG.replace('/config', '/diagnose')}`,
      );
      const result = await response.json();
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">配置和管理S3文件存储服务</p>
        </div>

        {/* 状态指示器 */}
        <div
          className={`flex items-center gap-2 px-3 py-2 rounded-lg bg-gray-50 ${getStatusColor()}`}
        >
          {getStatusIcon()}
          <span className="text-sm font-medium">{status?.message || '状态未知'}</span>
        </div>
      </div>
      ...
      <div className="mt-6">
        ...
        {activeTab === 'stats' && <StorageStatsTab stats={stats} />}
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white shadow rounded-lg">
        <div className="px-6 py-4 border-b border-gray-200">
          ...
          <p className="mt-1 text-sm text-gray-500">选择存储配置模式：自动配置或自定义配置</p>
        </div>
                ...
                <div className="ml-3">
                  ...
                  <div className="mt-2 text-sm text-yellow-700">
                    <ul className="list-disc list-inside space-y-1">
                      ...
                      <li>
                        将创建必要的存储桶（avatars、transaction-attachments、temp-files、system-files）
                      </li>
                      <li>配置将自动保存到数据库</li>
                    </ul>
                  </div>
      ...
      {configMode === 'custom' && templates && (
        <div className="bg-white shadow rounded-lg">
          <div className="px-6 py-4 border-b border-gray-200">
            ...
            <p className="mt-1 text-sm text-gray-500">选择预设模板快速配置存储服务</p>
          </div>
      ...
      <div className="bg-white shadow rounded-lg">
        <div className="px-6 py-4 border-b border-gray-200">
          ...
          <p className="mt-1 text-sm text-gray-500">配置S3存储服务的连接参数和存储桶设置</p>
        </div>
                  ...
                  onChange={(e) =>
                    setFormData({
                      ...
                      imageCompression: {
                        ...
                        avatar: formData.imageCompression?.avatar || {
                          enabled: true,
                          quality: 85,
                          maxWidth: 512,
                          maxHeight: 512,
                        ...
                        attachment: formData.imageCompression?.attachment || {
                          enabled: true,
                          quality: 80,
                          maxWidth: 1920,
                          maxHeight: 1920,
                        ...
                        multimodal: formData.imageCompression?.multimodal || {
                          enabled: true,
                          quality: 90,
                          maxWidth: 2048,
                          maxHeight: 2048,
                        ...
                        general: formData.imageCompression?.general || {
                          enabled: true,
                          quality: 80,
                          maxWidth: 1920,
                          maxHeight: 1920,
        ...
        <div className="bg-white shadow rounded-lg p-6">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="text-2xl font-semibold text-gray-900">
                {stats.bucketInfo
                  ? `${stats.bucketInfo.existing}/${stats.bucketInfo.configured}`
                  : '0'}
              </p>
              {stats.bucketInfo && <p className="text-xs text-gray-400">已存在/已配置</p>}
            </div>
          </div>
        </div>
      </div>
              ...
              stats.bucketInfo.buckets.map((bucket) => (
                ...
                >
                  <div className="flex items-center space-x-3">
                    <div
                      className={`w-3 h-3 rounded-full ${bucket.exists ? 'bg-green-500' : 'bg-red-500'}`}
                    ></div>
                    <div>
                  ...
                  <div className="text-right">
                    <p
                      className={`text-sm font-medium ${bucket.exists ? 'text-green-600' : 'text-red-600'}`}
                    >
