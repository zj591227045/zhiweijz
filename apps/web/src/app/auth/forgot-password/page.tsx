...
export default function ForgotPasswordPage() {
  ...
  const onSubmit = async (data: ForgotPasswordFormValues) => {
    try {
      ...
      toast.error(error.response?.data?.error?.message || '发送重置密码链接失败，请稍后再试');
    } finally {
      setIsLoading(false);
    }
  };
        ...
        <div className="auth-form flex flex-col gap-4 sm:gap-5 w-full max-w-[95%] sm:max-w-sm md:max-w-md mx-auto">
          ...
          {isSubmitted ? (
            ...
            <form
              ...
              className="space-y-4 sm:space-y-6 bg-white dark:bg-gray-800 p-4 sm:p-6 md:p-8 rounded-lg shadow-md box-border"
            >
