...
export const useBudgetStatisticsStore = create<BudgetStatisticsState>()(
  devtools((set, get) => ({
    ...
    // 获取用户级别的预算结转历史（简化版本）
    fetchUserRolloverHistory: async (accountBookId, budgetType = 'PERSONAL', targetUserId) => {
      try {
        console.log(
          `获取用户级别预算结转历史，账本ID: ${accountBookId}, 预算类型: ${budgetType}, 目标用户ID: ${targetUserId || '当前用户'}`,
        );
        const response = await budgetService.getUserBudgetRolloverHistory(
          accountBookId,
          budgetType,
          targetUserId,
        );
      ...
      try {
        ...
        if (filteredBudgetCards.length > 0) {
          ...
          if (
            ...
            try {
              ...
              if (currentUser) {
                // 查找当前用户对应的家庭成员
                const currentUserMember = response.familyMembers.find(
                  (member) => !member.isCustodial && member.name === currentUser.name,
                );
        ...
        if (defaultSelectedBudgetId) {
          ...
          if (
            ...
            const selectedMember = response.familyMembers.find(
              (member) => member.budgetId === defaultSelectedBudgetId,
            );
            if (selectedMember) {
              get().fetchBudgetTrends(
                defaultSelectedBudgetId,
                get().chartViewMode,
                get().chartTimeRange,
                selectedMember.id,
              );
