...
export function OfficialAIService() {
  ...
  const todayUsage =
    activeService?.type === 'official'
      ? {
          ...
          remainingTokens:
            (activeService.dailyTokenLimit || 50000) - (activeService.usedTokens || 0),
          usagePercentage: Math.round(
            ((activeService.usedTokens || 0) / (activeService.dailyTokenLimit || 50000)) * 100,
          ),
          totalCalls: 0,
          successfulCalls: 0,
          failedCalls: 0,
  ...
  const usageError =
    activeService?.type !== 'official' && !isLoadingActiveService
      ? '当前未启用官方AI服务'
      : activeServiceError;
        ...
        {isLoadingStatus ? (
          ...
          <div
            style={{
              ...
              alignItems: 'center',
              padding: '12px',
              backgroundColor: serviceStatus.isOnline
                ? 'rgba(34, 197, 94, 0.1)'
                : 'rgba(239, 68, 68, 0.1)',
              borderRadius: '8px',
            }}
