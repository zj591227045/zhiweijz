...
export function TagAnalysisPage() {
  ...
  // 准备图表数据
  const chartData =
    statisticsData?.data?.tagStatistics
      ?.map((item) => {
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-white p-3 border border-gray-200 rounded-lg shadow-lg">
          ...
          <p className="text-sm text-gray-600">金额: {formatCurrency(data.value)}</p>
          <p className="text-sm text-gray-600">记账数: {data.count} 笔</p>
        </div>
      );
    }
    return null;
      ...
      <div className="tag-analysis-page space-y-4 pb-20">
        ...
        {isLoading ? (
                          ...
                          <div key={index} className="flex items-center justify-between text-sm">
                            ...
                            <div className="text-right ml-2">
                              ...
                              <div className="text-xs text-gray-500">
                                {(
                                  (item.value / statisticsData.data.overview.totalAmount) *
                                  100
                                ).toFixed(1)}
                                %
                              </div>
          ...
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-12 text-center">
            ...
            <p className="text-gray-500 mb-4">在选定的时间范围内没有使用标签的记账记录</p>
