...
export function useVersionCheck(): UseVersionCheckReturn {
  ...
  // 检查版本更新
  const checkVersion = useCallback(async (params: VersionCheckRequest) => {
  ...
  // 设置用户版本状态
  const setUserVersionStatus = useCallback(async (action: 'postpone' | 'ignore' | 'update') => {
    ...
    try {
      ...
      // 根据平台确定
      const platform = window.location.hostname === 'localhost' ? 'web' : 
                     /android/i.test(navigator.userAgent) ? 'android' : 
                     /iphone|ipad/i.test(navigator.userAgent) ? 'ios' : 'web';

      const postponedUntil = action === 'postpone' ? 
        new Date(Date.now() + 7 * 24 * 60 * 60 * 1000) : undefined; // 推迟7天
