...
import { Budget, useTransactionEditStore } from "@/store/transaction-edit-store";
import { apiClient } from "@/lib/api";

// 获取活跃预算列表
const getActiveBudgets = async (accountBookId: string | null) => {
...
export function BudgetSelector() {
  ...
  const { data: budgetsData, isLoading } = useQuery({
    queryKey: ['activeBudgets', accountBookId],
    queryFn: () => getActiveBudgets(accountBookId),
  ...
  useEffect(() => {
    if (budgetsData) {
      ...
      // 如果有budgetId但没有selectedBudget，则从加载的预算中查找并选择
      const budgetId = useTransactionEditStore.getState().budgetId;
      if (budgetId) {
        const matchedBudget = budgetsData.find(b => b.id === budgetId);
        if (matchedBudget) {
          setSelectedBudget(matchedBudget);
        }
      }
    }
  }, [budgetsData, setBudgets, setSelectedBudget, selectedBudget]);
