...
**已创建预算结转金额修复脚本** - 根据历史记录更新最新预算的结转金额

**功能：**
- 查找每个用户/成员的最新预算（启用结转的个人预算）
- 从上一期预算的结转历史记录中获取结转金额
- 更新最新预算的 `rollover_amount` 字段
- 确保最新预算的结转金额与历史记录一致

**执行逻辑：**
1. 查找每个用户的最新预算
2. 查找该预算的上一期预算
3. 从 `budget_histories` 表中获取上期预算的结转历史记录
4. 如果没有历史记录，则手动计算：`结转金额 = 预算金额 + 上期结转金额 - 实际支出`
5. 更新最新预算的结转金额

**使用场景：**
- 当预算结转历史记录已修复，但最新预算的结转金额未同步时
- 建议先执行脚本2（修复预算结转历史记录），再执行此脚本
- 定期检查和修复最新预算的结转金额准确性

**特点：**
- 只处理最新预算（每个用户+账本组合的最新一期）
- 依赖预算结转历史记录（由脚本2生成）
- 允许0.01元的浮点数误差
- 提供详细的修复报告
