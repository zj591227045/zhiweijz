...
DECLARE
    ...
    v_user_name TEXT;
BEGIN
    SELECT EXISTS(SELECT 1 FROM users WHERE id = :user_id::uuid) INTO v_user_exists;
    
    IF NOT v_user_exists THEN
        RAISE EXCEPTION '用户不存在: %', :user_id;
    END IF;
    
    SELECT name INTO v_user_name FROM users WHERE id = :user_id::uuid;
    RAISE NOTICE '✓ 用户存在: % (ID: %)', v_user_name, :user_id;
END $$;
...
BEGIN
    SELECT 
        EXISTS(SELECT 1 FROM user_checkins WHERE user_id = :user_id::uuid AND checkin_date = :checkin_date::date),
        id,
        points_awarded
    INTO v_checkin_exists, v_existing_id, v_existing_points
    FROM user_checkins 
    WHERE user_id = :user_id::uuid AND checkin_date = :checkin_date::date;
...
BEGIN
    ...
    WHERE uap.user_id = :user_id::uuid
...
BEGIN
    SELECT 
        ...
        uap.member_balance,
        COUNT(uc.id)
    INTO v_user_name, v_total_balance, v_gift_balance, v_member_balance, v_total_checkins
    FROM users u
    LEFT JOIN user_accounting_points uap ON u.id = uap.user_id
    LEFT JOIN user_checkins uc ON u.id = uc.user_id
    WHERE u.id = :user_id::uuid
    GROUP BY u.name, uap.total_balance, uap.gift_balance, uap.member_balance;
