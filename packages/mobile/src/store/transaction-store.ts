...
export const useTransactionStore = createTransactionStore({
  ...
  onCreateSuccess: (transaction) => {
    // 移动端特定的创建成功处理
    console.log('记账创建成功:', transaction);
    // 不显示Alert，避免打断用户体验
  },
  onCreateError: (error) => {
    // 移动端特定的创建错误处理
    console.error('记账创建失败:', error);
    // 错误处理在组件中进行
  },
  onUpdateSuccess: (transaction) => {
    // 移动端特定的更新成功处理
    console.log('记账更新成功:', transaction);
    // 不显示Alert，避免打断用户体验
  },
  onUpdateError: (error) => {
    // 移动端特定的更新错误处理
    console.error('记账更新失败:', error);
    // 错误处理在组件中进行
  },
  onDeleteSuccess: () => {
    // 移动端特定的删除成功处理
